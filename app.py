...
class WebShell:
    ...
    def change_directory(self, command):
        ...
        """
        try:
            target_dir = command.split(' ', 1)[1] if len(command.split(' ', 1)) > 1 else os.path.expanduser('~')
...
class WebShellApp:
    ...
    def __init__(self, console_height=400, fixed_height=True):
        """
...
if __name__ == '__main__':
    ...
    web_shell_app = WebShellApp(console_height=600, fixed_height=False)  # Set the console height to 300 pixels
    web_shell_app.run()
